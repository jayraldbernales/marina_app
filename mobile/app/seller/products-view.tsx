...
// Updated freshness label computation for harvested_at
function computeFreshnessLabel(harvestedAt?: string | null) {
  if (!harvestedAt) return { text: "Unknown", color: "#6b7280" };

  const then = new Date(harvestedAt);
  const now = new Date();
  const diffMs = now.getTime() - then.getTime();
  const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));

  if (diffDays === 0) return { text: "Harvested Today", color: "#10b981" };
  if (diffDays === 1) return { text: "Harvested Yesterday", color: "#f59e0b" };
  if (diffDays >= 2 && diffDays <= 3)
    return { text: "2-3 Days Ago", color: "#d97706" };
  if (diffDays >= 4 && diffDays <= 7)
    return { text: "4-7 Days Ago", color: "#f97316" };
  return { text: "Over a Week", color: "#dc2626" };
}
...
export default function ProductView() {
  ...
  const freshness = computeFreshnessLabel(product?.harvested_at);
  const images = product?.images || [];
