...
export function useAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const chartData: ChartData[] = Object.keys(days).map((k) => ({
          name: k,
        ...
        // ============ FIXED: RECENT ORDERS - NO COMPLEX JOINS ============
        ...
        if (!ordersData || ordersData.length === 0) {
          ...
          // Get buyer names from profiles separately
        ...
        // ============ FIXED: TOP VENDORS ============
        // Get all vendors
        const { data: vendorsList, error: vendorsError } = await supabase.from(
          "vendor_profiles",
        ).select(`
            user_id,
            shop_name,
        ...
        if (!vendorsList || vendorsList.length === 0) {
          ...
          const topVendorsSorted = vendorsMapped
            .sort((a, b) => b.totalSales - a.totalSales)
            .slice(0, 5);

          if (!mounted) return;
          setTopVendors(topVendorsSorted);
        }
      } catch (err: any) {
  ...
  return { stats, weeklyData, recentOrders, topVendors, loading, error };
}
